...
function wifi-fakeauth() {
        ...
        while getopts :c:d:r:h OPTION; do
            case "${OPTION}" in
                ...
                h) echo_warning 'wifi-fakeauth';
                    echo_label 14 '  description:'; echo_primary 'Associates with target wifi network'
                    echo_label 14 '  usage:'; echo_primary 'wifi-fakeauth (adapter) -r [router_mac] (-c client_mac) (-d delay) -h (help)'
    ...
    if [ "${#ARGUMENTS[@]}" -gt 1 ]; then
        ...
        echo_label 8 'usage:'; echo_primary 'wifi-fakeauth (adapter) -r [router_mac] (-c client_mac) (-d delay) -h (help)'
