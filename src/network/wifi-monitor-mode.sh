...
function wifi-monitor-mode() {
        ...
        while getopts :h OPTION; do
            case "${OPTION}" in
                h) echo_warning 'wifi-monitor-mode';
                    echo_label 14 '  description:'; echo_primary 'Set wlan adapter to monitor mode'
                    echo_label 14 '  usage:'; echo_primary 'wifi-monitor-mode (adapter) -h (help)'
    ...
    if [ "${#ARGUMENTS[@]}" -gt 1 ]; then
        ...
        echo_label 8 'usage:'; echo_primary 'wifi-monitor-mode (adapter) -h (help)'
