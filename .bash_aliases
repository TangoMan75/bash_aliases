...
# shellcheck disable=SC2034
APP_INSTALL_DIR=~/Documents/bash_aliases

# app user config directory
# shellcheck disable=SC2034
APP_USER_CONFIG_DIR=~/.TangoMan75/bash_aliases/config
...
alias _abort='git-abort' ## git-abort alias

## Abort the rebase or pick operation
function git-abort() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'git-abort -h (help)\n'
    }
...
function convert-to-mp3() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'convert-to-mp3 [folder] -h (help)\n'
    }
    ...
    find "${source}" -type f -iregex ".+\.\(aac\|aiff\|flac\|m4a\|mp3\|ogg\|wav\|wma\|avi\|flv\|m2ts\|mkv\|mov\|mp4\|mts\|webm\|wmv\)" | while read -r file_path; do
        ...
        # check file codec
        if [ "${codec}" = 'mp3' ]; then
            echo_warning "ignored : \"${file_path}\" - already mp3 format\n"
            continue
        fi
        ...
        echo_info "ffmpeg -nostdin -hide_banner -i \"${file_path}\" \"${new_path}\"\n"
        ffmpeg -nostdin -hide_banner -i "${file_path}" "${new_path}"
    done
}
...
## Resize JPG images
function picture-jpeg-resize() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-jpeg-resize [folder] -s (size default=50%) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :sh option; do
            case "${option}" in
                s) size="${OPTARG}";;
                h) echo_warning 'picture-jpeg-resize\n';
                    echo_success 'description:' 2 14; echo_primary 'Resize JPG images\n'
                    _usage 2 14
                    return 0;;
                :) echo_error "\"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    find "${source}" -type f -iregex ".+\.jpe?g" | while read -r file_path; do
        # resize image
        echo_info "convert -resize \"${size}\" \"${file_path}\" \"${file_path}\"\n"
        convert -resize "${size}" "${file_path}" "${file_path}"
    done
}
...
function picture-png2jpg() {
    ...
    find "${source}" -type f -name '*.png' | while read -r file_path; do
        ...
        # convert to jpeg
        echo_info "convert \"${file_path}\" \"${new_file_path}\"\n"
        convert "${file_path}" "${new_file_path}"
    done
}
