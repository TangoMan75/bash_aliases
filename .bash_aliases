...
## Create ".env" file into "~/config/tangoman/bash_aliases" folder
...
function _load_env() {
    ...
    if [ ! -f "${file_path}" ]; then
        echo_error "\"${file_path}\" file not found\n"
        _usage 2 8
        return 1
    fi
...
function open-in-ide() {
    ...
    if [ ! -x "$(command -v phpstorm-url-handler)" ]; then
        echo_error 'ide-url-handler required, visit: "https://github.com/TangoMan75/ide-url-handler" to install\n'
        return 1
    fi
    ...
    if [ -n "${line}" ]; then
        if [ "${line}" -lt 1 ] || [[ ! "${line}" =~ ^[0-9]+$ ]]; then
            echo_error '"line" should be a positive integer\n'
            _usage 2 8
            return 1
        fi
    fi
...
function _parse_commit_scope() {
    ...
    if [ ! -x "$(command -v awk)" ]; then
        echo_error 'awk required, enter: "sudo apt-get install -y awk" to install\n'
        return 1
    fi
...
function _pick_repository() {
    ...
    if [ "${#repositories[@]}" -eq 0 ]; then
        echo_error 'No repositorys found\n'
        return 1;
    fi
...
function blame() {
    ...
    if [ -n "${commit_hash}" ]; then
        if [ "$(_commit_exists "${arguments[${LBOUND}]}")" = false ]; then
            echo_error "Invalid commit hash : \"${OBJECT}\"\n"
            _usage
            return 1
        fi
    fi
    ...
    if [ -n "${number}" ]; then
        if [ "${number}" -lt 1 ] || [[ ! "${number}" =~ ^[0-9]+$ ]]; then
            echo_error '"number" should be a positive integer\n'
            _usage
            return 1
        fi
    fi
...
function checkout() {
    ...
    if [ -n "${arguments[${LBOUND}]}" ]; then
        ...
        if [ "$(_branch_exists "${object}")" = false ] && [ "$(_commit_exists "${object}")" = false ]; then
            echo_error "Invalid commit or branch name : \"${object}\"\n"
            _usage
            return 1
        fi
    fi
...
function clone() {
    ...
    if [ -z "${GIT_SERVER}" ] || [ "${GIT_SERVER}" = default_git_server ]; then
        echo_error 'missing git server\n'
        _usage 2 8
        return 1
    fi

    if [ "${GIT_SERVER}" != 'gist.github.com' ]; then
        if [ -z "${GIT_USERNAME}" ] || [ "${GIT_USERNAME}" = default_git_username ]; then
            echo_error 'missing git username\n'
            _usage
            return 1
        fi
    fi
...
function get_url() {
    ...
    if [ -z "${repository_url}" ]; then
        echo_error "remote get-url is not available in local git repository\n"
        return 1
    fi
    ...
    if echo "${repository_url}" | grep -q -E '^(http://|https://|git@)'; then
        ...
        echo_error "unknown url format in local git repository\n"
        return 1
    fi
...
function greset() {
    ...
    if [ -n "${commit_hash}" ]; then
        if [ "$(_commit_exists "${commit_hash}")" = false ]; then
            echo_error "Invalid commit : \"${commit_hash}\"\n"
            _usage
            return 1
        fi
    fi
...
function convert-to-mp3() {
    ...
    function _convert() {
        ...
        if [ -z "${CODEC}" ]; then
            echo_error "cannot find valid codec for \"$1\"\n"
    ...
    if [ -z "$(command -v 'ffmpeg')" ]; then
        echo_error 'ffmpeg not installed, try "sudo apt-get install -y ffmpeg"\n'
        return 1
    fi
    ...
    if [ ! -f "${source}" ] && [ ! -d "${source}" ]; then
        echo_error "source: \"${source}\" is invalid\n"
        _usage 2 8
    ...
    if [ ! -d "${source}" ] && [ "${recursive}" = true ]; then
        echo_error "source: \"${source}\" is not a valid directory\n"
        _usage 2 8
...
function picture-jpeg-optimize() {
    ...
    if [ -z "$(command -v 'jpegoptim')" ]; then
        echo_error 'jpegoptim not installed, try "sudo apt-get install -y jpegoptim"\n'
        return 1
    fi
...
function rename-extension-to-codec() {
    ...
    find "${source}" -type f -iregex ".+\.\(aac\|aiff\|flac\|m4a\|mp3\|ogg\|wav\|wma\|avi\|flv\|m2ts\|mkv\|mov\|mp4\|mts\|webm\|wmv\)" | while read -r file_path; do
        ...
        # check codec is valid
        if [ -z "${codec}" ]; then
            echo_error "cannot find valid codec for \"${file_path}\"\n"
...
function check-dns() {
    ...
    if [ -z "$(command -v 'dig')" ]; then
        echo_error 'dig not installed, try "sudo apt-get install -y dig"\n'
        return 1
    fi
...
function external-ip() {
    ...
    if [ -z "$(command -v 'wget')" ] && [ -z "$(command -v 'curl')" ]; then
        echo_error 'curl not installed, try "sudo apt-get install -y curl"\n'
        return 1
    fi
...
function hosts() {
    ...
    if [ -x "$(command -v "${VISUAL}")" ]; then
        echo_error "${VISUAL} not installed, try \"sudo apt-get install -y ${VISUAL}\"\n"
        return 1
    fi
...
function list-adapters() {
    ...
    if [ "${filter_connected}" = true ]; then
        ...
        if [ -z "$(command -v 'arp')" ]; then
            echo_error 'arp not installed, try "sudo apt-get install -y net-tools"\n'
            return 1
        fi
        ...
        if [ -z "$(command -v 'iwconfig')" ]; then
            echo_error 'iwconfig not installed, try "sudo apt-get install -y wireless-tools"\n'
            return 1
        fi
        ...
        if [ -x "$(command -v 'ip')" ]; then
            ...
            echo_error 'ifconfig not installed, try "sudo apt-get install -y net-tools"\n'
            return 1
        fi
...
function set-php-version() {
    ...
    if [ "${is_valid}" = false ]; then
        echo_error 'Invalid PHP version\n'
        _usage 2 8
        return 1
    fi
...
function change-mac-address() {
    ...
    if [[ ! "${spoof}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        echo_error "invalid mac address \"${spoof}\"\n"
        _usage 2 8
...
function sf-server() {
    ...
    if [ -z "$(command -v symfony)" ]; then
        echo_error 'symfony cli binary required\n'
        return 1
    fi
...
function flash-bootable-iso() {
    ...
    if [ -z "${source}" ]; then
        echo_error 'source parameter is missing\n'
        _usage
    fi

    if [ -z "${destination}" ]; then
        echo_error 'destination parameter is missing\n'
        _usage
    fi
...
function change-extensions() {
    ...
    find "${source}" -maxdepth 1 -type f -name "*.${old_extension}" | while read -r file_path; do
        ...
        if [ -f "${new_path}" ]; then
            echo_error 'file already exists.\n'
        else
            # rename file
            echo_info "mv -nv \"${file_path}\" \"${new_path}\"\n"
            mv -nv "${file_path}" "${new_path}"
...
function compress() {
    ...
    if [ ! -d "${destination}" ]; then
        echo_error "\"${destination}\" is not a valid directory\n"
        _usage 2 8
        return 1
    fi
...
function create-desktop-shortcut() {
    ...
    if [ ! -x "${source}" ]; then
        echo_error "\"${source}\" is not executable\n"
        _usage 2 8
        return 1
    fi
