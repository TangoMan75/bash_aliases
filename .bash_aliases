...
## Print clickable hyperlink using OSC 8 escape sequence
_echo_link() {
    #   URL:  Target URL
...
## Print semantic color test
function tango-colors() {
    ...
    _echo_light     '_echo_light\n'
    _echo_dark      '_echo_dark\n'
...
if [ -x "$(command -v opencode)" ]; then
    alias oc='opencode'  ## Start OpenCode
fi
...
if [ -x "$(command -v rg)" ]; then
    alias hl='rg --passthrough'  ## Highlight text
fi
...
function docker-status() {
    ...
    _echo_info "docker inspect --format '{{slice .ID 0 13}} {{slice .Name 1}} {{range .NetworkSettings.Networks}}{{if .IPAddress}}http://{{.IPAddress}} {{end}}{{end}}{{range \$p, \$c := .NetworkSettings.Ports}}{{\$p}} {{end}}' \$(docker ps --quiet) | column -t\n"
    # shellcheck disable=SC2046
    docker inspect --format '{{slice .ID 0 13}} {{slice .Name 1}} {{range .NetworkSettings.Networks}}{{if .IPAddress}}http://{{.IPAddress}} {{end}}{{end}}{{range $p, $c := .NetworkSettings.Ports}}{{$p}} {{end}}' $(docker ps --quiet) | column -t
}
...
## Check blob exists
function _blob_exists() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary '_blob_exists [blob_hash] -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :vh option; do
            case "${option}" in
                v) verbose=true;;
                h) _echo_warning '_blob_exists\n';
                    _echo_success 'description:' 2 14; _echo_primary 'Check blob exists\n'
                    _usage 2 14
                    return 0;;
                \?) _echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    blob_hash="${arguments[${LBOUND}]}"

    #--------------------------------------------------
    # Build command
    #--------------------------------------------------

    command="git cat-file blob ${blob_hash} &>/dev/null && echo true || echo false"
...
# Sanitize string for safe eval (escape backslash, backtick, double quote, dollar)
function _sanitize() {
    local sanitized="${1//\\/\\\\}"   # \ -> \\
    sanitized="${sanitized//\`/\\\`}" # ` -> \`
    sanitized="${sanitized//\"/\\\"}" # " -> \"
...
function branch() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'branch (name) -F [filter branches] -i (interactive) -l (list) -a (list all) -v (verbose) -f (fetch) -p (prune) -d (delete) -D (delete remote) -r (rename) -u (set upstream) -A (all, fetch and prune) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        OPTIND=0
        while getopts :aAdDfF:ilpruvh option; do
            case "${option}" in
                a) list_all=true;;
    ...
    if [ "${list}" = true ] && [ "${set_upstream}" = false ]; then
        ...
        if [ "${list_all}" = true ]; then
            branch_flags="-a${branch_flags}"
        elif [ -n "${branch_flags}" ]; then
            branch_flags="-${branch_flags}"
        fi
        _echo_info "git --no-pager branch ${branch_flags}\n"
        # shellcheck disable=SC2086
        git --no-pager branch ${branch_flags}
        return 0
    fi
...
function conventional-commit() {
    ...
    if [ -n "${author}" ]; then
        author="--author \"$(_sanitize "${author}")\""
    fi
...
function delete-github-deployments() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'delete-github-deployments -h (help)\n'
    }
    ...
    while getopts :h option; do
        case "${option}" in
            h) _echo_warning 'delete-github-deployments\n';
                _echo_success 'description:' 2 14; _echo_primary 'Delete old Github deployments\n'
                _usage 2 14
                return 0;;
            \?) _echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
    ...
    _echo_info "gh api \"repos/${git_username}/${git_repository}/deployments\" | jq -r '.[].id' | while read -r id;\n"
    gh api "repos/${git_username}/${git_repository}/deployments" | jq -r '.[].id' | while read -r id;
    do
        _echo_info "gh api \"repos/${git_username}/${git_repository}/deployments/${id}\" -X DELETE\n"
        gh api "repos/${git_username}/${git_repository}/deployments/${id}" -X DELETE
    done
}
...
function delete-github-workflows() {
    ...
    _echo_info "gh api \"repos/${git_username}/${git_repository}/actions/runs\" | jq -r '.workflow_runs[].id' | while read -r id;\n"
...
## Show changes between commits, branches, commit and working tree, etc
function gdiff() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'gdiff (ref1 ref2 | branch_name/commit_hash) -c (color) -f [file path] -F [file type filter] -n [number] -B (pick branch) -C (pick commit) -l (list files only) -m (diff with main branch) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        OPTIND=0
        while getopts :123456789b:BcCf:F:lmn:vh option; do
            case "${option}" in
                [1-9]) number="${option}";;
                B) pick_branch=true;;
                c) color=true;;
                ...
                h) _echo_warning 'gdiff\n';
                    _echo_success 'description:' 2 14; _echo_primary 'Show changes between commits, branches, commit and working tree, etc\n'
                    _usage 2 14
                    return 0;;
                :) _echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    # Second argument could be a branch name or a commit hash
    if [ -n "${arguments[${LBOUND}+1]}" ]; then
        object2="${arguments[${LBOUND}+1]}"

        if [ "$(_branch_exists "${object2}")" = true ]; then
            branch2="${arguments[${LBOUND}+1]}"

        elif [ "$(_commit_exists "${object2}")" = true ]; then
            commit_hash2="${arguments[${LBOUND}+1]}"

        else
            _echo_danger "error: Invalid commit or branch name : \"${object2}\"\n"
            _usage
            return 1
        fi
    fi
    ...
    # pickers are silently ignored when two references are provided
    if [ -z "${branch2}" ] && [ -z "${commit_hash2}" ]; then
        # ignored when "commit hash" argument set
        if [ "${pick_branch}" = true ] && [ -z "${commit_hash}" ] && [ -z "${branch}" ]; then
            branch="$(_pick_branch -m)"
        fi
    ...
    # when two references are provided, use them as-is
    if [ -n "${branch2}" ] || [ -n "${commit_hash2}" ]; then
    ...
    if [ -n "${branch2}" ] || [ -n "${commit_hash2}" ]; then
        command="${command} ${branch:-${commit_hash}}..${branch2:-${commit_hash2}}"

    elif [ "${diff_with_branch}" = true ]; then
...
function get_url() {
    ...
    if [ "${get_pull_request}" = true ]; then
        if [ "${git_server}" = 'github.com' ] && [ -x "$(command -v gh)" ] && [ -x "$(command -v jq)" ]; then
            ...
            gh pr view --json url 2>/dev/null | jq -r '.url'
            return 0
        fi
...
alias iopen='open-commit-in-ide' ## open-commit-in-ide alias
alias open-conflicts-in-ide='open-commit-in-ide' ## open-commit-in-ide alias

## Open modified/conflicting files in ide
function open-commit-in-ide() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'open-commit-in-ide (branch_name/commit_hash) -F [file type filter] -i [ide] -n [number] -B (pick branch) -C (pick commit)  -I (pick ide) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :123456789BCF:i:In:vh option; do
            case "${option}" in
                ...
                n) number="${OPTARG}";;
                v) verbose=true;;
                h) _echo_warning 'open-commit-in-ide\n';
...
function pull-request() {
    ...
    if [ "${comments}" = true ]; then
        _echo_info "gh api \"repos/${git_username}/${git_repository}/pulls/${pr_number}/comments\"\n"
...
## Print changes from given commit or prints blob content
function show() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'show (branch_name/commit_hash) -c (color) -f [file path] -F [file type filter] -n [number] -B (pick branch) -C (pick commit) -l (list files only) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        OPTIND=0
        while getopts :123456789BCcf:F:ln:vh option; do
            case "${option}" in
                [1-9]) number="${option}";;
                B) pick_branch=true;;
                C) pick_commit=true;;
                ...
                v) verbose=true;;
                h) _echo_warning 'show\n';
                    _echo_success 'description:' 2 14; _echo_primary 'Print changes from given commit or prints blob content\n'
                    _usage 2 14
                    return 0;;
                :) _echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -n "${arguments[${LBOUND}]}" ]; then
        ...
        elif [ "$(_blob_exists "${object}")" = true ]; then
            blob_hash="${arguments[${LBOUND}]}"

        else
            _echo_danger "error: Invalid commit, branch name, or blob hash : \"${object}\"\n"
            _usage
            return 1
        fi
    fi
    ...
    # ignored when "branch" argument or "blob" argument is set
    if [ "${pick_branch}" = true ] && [ -z "${branch}" ] && [ -z "${blob_hash}" ]; then
        branch="$(_pick_branch -m)"
    fi
    # shellcheck disable=2181
    if [ $? != 0 ]; then
    ...
    # ignored when "commit hash" argument or "blob" argument is set
    if [ "${pick_commit}" = true ] && [ -z "${commit_hash}" ] && [ -z "${blob_hash}" ]; then
        commit_hash=$(_pick_commit "${branch}")
    fi
    # shellcheck disable=2181
    if [ $? != 0 ]; then
    ...
    #--------------------------------------------------
    # Validate blob options
    #--------------------------------------------------

    if [ -n "${blob_hash}" ]; then
        if [ "${list_files_only}" = true ]; then
            _echo_danger 'error: "-l" (list files) is not compatible with blob hash\n'
            _usage
            return 1
        fi
        if [ -n "${filter}" ]; then
            _echo_danger 'error: "-F" (file type filter) is not compatible with blob hash\n'
            _usage
            return 1
        fi
        if [ -n "${file_path}" ]; then
            _echo_danger 'error: "-f" (file path) is not compatible with blob hash\n'
            _usage
            return 1
        fi
        if [ -n "${number}" ]; then
            _echo_danger 'error: "-n" (number) is not compatible with blob hash\n'
            _usage
            return 1
        fi
    fi
    ...
    if [ -z "${commit_hash}" ] && [ -z "${blob_hash}" ]; then
        commit_hash=HEAD
    fi
    ...
    if [ -n "${blob_hash}" ]; then
        command="${command} ${blob_hash}"

    elif [ -n "${branch}" ]; then
        command="${command} ${branch}"

    elif [ -n "${commit_hash}" ] && [ "${commit_hash}" != 'HEAD' ]; then
        command="${command} ${commit_hash}"
    fi
...
function picture-organize() {
    ...
    find "${source}" -type f -iregex '.+\.\(3g2\|3gp\|aax\|ai\|arw\|asf\|avi\|cr2\|cr3\|crm\|crw\|cs1\|dcp\|dng\|dr4\|dvb\|eps\|erf\|exif\|exv\|f4a\|f4v\|fff\|flif\|gif\|gpr\|hdp\|icc\|iiq\|ind\|jng\|jp2\|jpeg\|jpg\|m4a\|m4v\|mef\|mie\|mng\|mod\|mos\|mov\|mp4\|mpeg\|mpg\|mpo\|mqv\|mrw\|nef\|nrw\|orf\|pbm\|pdf\|pef\|pgm\|png\|png\|ppm\|ps\|psb\|psd\|qtif\|raf\|raw\|rw2\|rwl\|sr2\|srw\|thm\|tiff\|vrd\|wdp\|wmv\|x3f\|xmp\)$' | while read -r file_path; do
        ...
        if [ "${camera}" = true ]; then
            ...
            folder="${folder}_-_${USER}_-_${camera_model}"
        fi
...
function picture-rename-to-datetime() {
    ...
    find "${source}" -type f -iregex '.+\.\(3g2\|3gp\|aax\|ai\|arw\|asf\|avi\|cr2\|cr3\|crm\|crw\|cs1\|dcp\|dng\|dr4\|dvb\|eps\|erf\|exif\|exv\|f4a\|f4v\|fff\|flif\|gif\|gpr\|hdp\|icc\|iiq\|ind\|jng\|jp2\|jpeg\|jpg\|m4a\|m4v\|mef\|mie\|mng\|mod\|mos\|mov\|mp4\|mpeg\|mpg\|mpo\|mqv\|mrw\|nef\|nrw\|orf\|pbm\|pdf\|pef\|pgm\|png\|png\|ppm\|ps\|psb\|psd\|qtif\|raf\|raw\|rw2\|rwl\|sr2\|srw\|thm\|tiff\|vrd\|wdp\|wmv\|x3f\|xmp\)$' | sort -t '\0' -n | while read -r file_path; do
        ...
        if [ "${exif}" = true ]; then
            ...
            # -m  Ignore minor errors and warnings
            # -s3 print values only (no tag names)
            datetime="$(exiftool -m -P -s3 -d '%Y-%m-%d_%H-%M-%S' -DateTimeOriginal "${file_path}")"
        else
            # get datetime from file date last modified
            datetime="$(date '+%Y-%m-%d_%H-%M-%S' -r "${file_path}")"
        fi

        # validate date is not empty or invalid
        if ! echo "${datetime}" | grep -Eq '^[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}$'; then
            _echo_warning "\"${file_path}:\" invalid date \"${datetime}\" skipping\n"
            continue
        fi

        if [ "${append}" = true ]; then
            new_basename="${datetime}_${old_basename}"
        else
            new_basename="${datetime}"
        fi
...
function mod() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'mod (source) -m [mode] -F [filter] -r (recursive) -d (default=664) -x (executable=775) -X (executable=777) -s (sudo) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        OPTIND=0
        while getopts :drsxXm:F:h option; do
            case "${option}" in
                d) mode='664';;
                r) recursive=true;;
                s) use_sudo=false;;
    ...
    if [ -d "${source}" ] && [ "${recursive}" = true ]; then
        # Find all files recursively
        if [ -n "${filter}" ]; then
            find "${source}" -type f -name "*${filter}" | while read -r FILE
            do
    ...
    if [ -d "${source}" ]; then
        ...
        if [ -n "${filter}" ]; then
            find "${source}" -maxdepth 1 -type f -name "*${filter}" | while read -r FILE
            do
                _mod "${mode}" "${FILE}" "${use_sudo}"
            done
        else
...
function rename() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'rename [source] -r (recursive) -d (directories) -a (preserve accents) -e (remove extra char) -s (snake_case) -t (train-case) -l (lower case) -u (upper CASE) -h (help)\n'
    }
    ...
    if [ -d "${source}" ] && [ "${recursive}" = true ]; then
        if [ "${directories}" = true ]; then
            find "${source}" -type d ! -path "${source}" | sort -r | while read -r file_path; do
                _rename "${file_path}"
            done
        fi
