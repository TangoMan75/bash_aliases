...
function gstatus() {
    ...
    if [ "$(get_url -s)" = 'github.com' ]; then
        if [ "${verbose}" -ge 3 ]; then
            # print detailled github pull requests
            _echo_info "gh pr checks --json name,state,link -t '{{range .}}{{printf \"%-10s\\\t%-55s\\\t%s\\\n\" .state .name .link}}{{end}}' | column -t -s $'\\\t'\n"
            gh pr checks --json name,state,link -t '{{range .}}{{printf "%-10s\t%-55s\t%s\n" .state .name .link}}{{end}}' | column -t -s $'\t'
