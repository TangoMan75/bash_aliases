...
function open-in-ide() {
    ...
    if [ -z "$(command -v "${ide}")" ]; then
        echo_danger "error: invalid ide \"${ide}\"\n"
        _usage 2 8
        return 1
    fi
    ...
    if [ "${ide}" != "${DEFAULT_IDE}" ]; then
        echo_info "sed -i -E s/\"^DEFAULT_IDE=.*$/DEFAULT_IDE=${ide}\"/ ~/.bash_aliases\n"
    ...
    if [ "${#arguments[@]}" -gt 10 ]; then
        echo_warning "Are you sure you want to open ${#arguments[@]} tabs ? (yes/no) [no]: "
        read -r USER_PROMPT
        if [[ ! "${USER_PROMPT}" =~ ^[Yy](ES|es)?$ ]]; then
    ...
    for FILE_NAME in ${arguments[@]}; do
        ...
        if [ ! -f "${FILE_PATH}" ]; then
            echo_warning "\"${FILE_NAME}\" file not found\n"
            continue
        fi
        echo_info "(nohup xdg-open \"${ide}:open?url=file://${FILE_PATH}&line=${line}\" &>/dev/null &)\n"
...
function docker-clean() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'docker-clean -a (all) -c (container) -i (image)  -n (network) -s (system) -v (volume) -h (help)\n'
    }
    ...
    while getopts :acinsvh option; do
        case "${option}" in
            ...
            h) echo_warning 'docker-clean\n';
                echo_success 'description:' 2 14; echo_primary 'Remove unused containers, images, networks, system and volumes\n'
                _usage 2 14
                return 0;;
            \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
    ...
    if [ "${container}" = false ] && [ "${image}" = false ] && [ "${network}" = false ] && [ "${system}" = false ] && [ "${volume}" = false ]; then
        echo_danger 'error: no option given\n'
        _usage 2 14
        return 1
    fi
...
function docker-exec() {
    ...
    if [ "${compose}" = true ]; then
        if [ ! -x "$(command -v docker-compose)" ]; then
            echo_danger 'error: docker-compose required\n'
            return 1
        fi
    fi
...
function docker-list() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'docker-list -a (all) -d (use docker-compose) -h (help)\n'
    }
        ...
        while getopts :adh option; do
            case "${option}" in
                ...
                h) echo_warning 'docker-list\n';
                    echo_success 'description:' 2 14; echo_primary 'List running containers\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${all}" = true ]; then
        echo_info "docker ps --all --quiet --format '{{.Names}}'\n"
        docker ps --all --quiet --format '{{.Names}}'

        return 0
    fi

    echo_info "docker ps --format '{{.Names}}'\n"
...
function git-config() {
    ...
    alert_primary 'Config bash_aliases git default settings'
    ...
    echo_success "Please enter your Jira server domain name : [atlassian.com] "
    read -r JIRA_SERVER
    ...
    # prompt user values
    PS3=$(echo_success 'Please select default git server : ')
    ...
    while [ "${GIT_SERVER}" = other ] || [ -z "${GIT_SERVER}" ]; do
        echo_success 'Please enter default git server name : '
        read -r GIT_SERVER
    ...
    echo_success "Please enter your ${GIT_SERVER} user name : [${USER}] "
    read -r GIT_USERNAME
    ...
    echo_success "Please enter git config global user.email : [${DEFAULT_COMMIT_EMAIL}] "
    ...
    echo_success "Please enter git config global user.name : [${DEFAULT_COMMIT_NAME}] "
    read -r git_commit_name
    if [ -z "${git_commit_name}" ]; then
        git_commit_name="${DEFAULT_COMMIT_NAME}"
    fi
    echo_primary "  ${git_commit_name}\n"
    echo
    ...
    echo_success 'Use SSH ? (yes/no) [yes] : '
    read -r RESPONSE
    ...
    echo_info "git config --global user.email \"${git_commit_email}\"\n"
    git config --global user.email "${git_commit_email}"

    echo_info "git config --global user.name \"${git_commit_name}\"\n"
    ...
    echo_warning 'Current git configuration:\n'
    echo_success 'default jira server:' 2 24;   echo_primary "${JIRA_SERVER}\n"
    echo_success 'default git server:' 2 24;    echo_primary "${GIT_SERVER}\n"
    echo_success 'default git username:' 2 24;  echo_primary "${GIT_USERNAME}\n"
    echo_success 'git config user.email:' 2 24; echo_primary "$(git config user.email)\n"
    echo_success 'git config user.name:' 2 24;  echo_primary "$(git config user.name)\n"
    echo_success 'use ssh:' 2 24;               echo_primary "${GIT_SSH}\n"
    echo
    echo_warning 'You will need to reload your terminal for these changes to take effect.\n'
    echo
...
function _branch_exists() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary '_branch_exists [branch name] -a (list all) -r (remotes only) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :avrh option; do
            case "${option}" in
                ...
                r) remotes=true;;
                v) verbose=true;;
                h) echo_warning '_branch_exists\n';
                    echo_success 'description:' 2 14; echo_primary 'Check branch exists\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function _commit_exists() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary '_commit_exists [commit_hash] -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :vh option; do
            case "${option}" in
                v) verbose=true;;
                h) echo_warning '_commit_exists\n';
                    echo_success 'description:' 2 14; echo_primary 'Check commit exists\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function _parse_commit_scope() {
    ...
    if [ ! -x "$(command -v awk)" ]; then
        echo_danger 'error: awk required, enter: "sudo apt-get install -y awk" to install\n'
        return 1
    fi
...
function _pick_branch() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary '_pick_branch -F [file type filter] -a (list all) -m (list main) -c (list current) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :acF:mvh option; do
            case "${option}" in
                ...
                m) list_main=true;;
                v) verbose=true;;
                h) echo_warning '_pick_branch\n';
                    echo_success 'description:' 2 14; echo_primary 'Select a branch among multiple options\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ -z "${branches[${LBOUND}]}" ]; then
        echo_danger 'error: No branches found\n'
        return 1;
    fi

    #--------------------------------------------------

    PS3=$(echo_success 'Please select branch : ')
...
function _pick_commit() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary '_pick_commit (branch) -n [count] -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :n:vh option; do
            case "${option}" in
                n) max_count="${OPTARG}";;
                v) verbose=true;;
                h) echo_warning '_pick_commit\n';
                    echo_success 'description:' 2 14; echo_primary 'Select a commit among multiple options\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ -z "${commits[${LBOUND}]}" ]; then
        echo_danger 'error: No commits found\n'
        return 1;
    fi

    PS3=$(echo_success 'Please select commit : ')
...
function _pick_repository() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary '_pick_repository [user_name] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning '_pick_repository\n';
                    echo_success 'description:' 2 14; echo_primary 'Select a repository among multiple options\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${#repositories[@]}" -eq 0 ]; then
        echo_danger 'error: No repositorys found\n'
        return 1;
    fi

    PS3=$(echo_success 'Please select repository : ')
...
function add() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'add (files) -F [file type filter] -p (patch) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :F:pvh option; do
            case "${option}" in
                ...
                v) verbose=true;;
                h) echo_warning 'add\n';
                    echo_success 'description:' 2 14; echo_primary 'Stage files to git index\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function blame() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'blame [file path] -c [commit hash] -n [number] -B (pick branch) -C (pick commit) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        do
            case "${option}" in
                ...
                n) number="${OPTARG}";;
                h) echo_warning 'show\n';
                    echo_success 'description:' 2 14; echo_primary 'Show what revision and author last modified each line of a file\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -n "${commit_hash}" ]; then
        if [ "$(_commit_exists "${arguments[${LBOUND}]}")" = false ]; then
            echo_danger "error: Invalid commit hash : \"${OBJECT}\"\n"
            _usage
            return 1
        fi
    fi
...
function branch() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'branch (name) -F [file type filter] -i (interactive) -l (list) -a (list all) -f (fetch) -p (prune) -d (delete) -D (delete remote) -r (rename) -u (set upstream) -A (all, fetch and prune) -h (help)\n'
    }
        ...
        while getopts :aAdDfF:ilpruh option; do
            case "${option}" in
                ...
                h) echo_warning 'branch\n';
                    echo_success 'description:' 2 14; echo_primary 'Create, checkout, rename or delete branch\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${set_upstream}" = true ]; then
        # get current branch name
        echo_info "git branch --set-upstream-to=\"origin/${current_branch}\"\n"
    ...
    if [ "${delete}" = true ] || [ "${delete_remote}" = true ]; then
        alert_danger 'Are you sure you want to permanently delete this branch ?'
        if [ "${delete_remote}" = true ]; then
            echo_danger "This will remove \"${branch}\" from both local and remote repositories !\n"
        else
            echo_danger "This will remove \"${branch}\" from local repository !\n"
        fi
        echo_primary 'This action is irreversible : (yes/no) [no]: '
    ...
    if [ "${checkout}" = true ]; then
        ...
        if [ -z "$(git --no-pager branch --list "${branch}")" ]; then
            ...
            if [ -z "$(git --no-pager branch -r --list "origin/${branch}")" ]; then
                alert_success 'Creating new local branch'
                if [ "${interactive_rename}" = true ]; then
                ...
                alert_warning 'Fetching branch from remote'
                echo_info "git checkout origin/${branch} --track\n"
                git checkout "origin/${branch}" --track
            fi
        else
            echo_warning 'swith to local branch\n'
...
function checkout() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'checkout (branch_name/commit_hash) -f [file path] -B (pick branch) -C (pick commit) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :BCf:h option; do
            case "${option}" in
                ...
                C) pick_commit=true;;
                f) file_path="${OPTARG}";;
                h) echo_warning 'checkout\n';
                    echo_success 'description:' 2 14; echo_primary 'Restore working tree files\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ "${pick_branch}" = true ] && [ -z "${object}" ]; then
        echo_info 'git --no-pager branch --format="%(refname:short)"\n'
    ...
    if [ "${object}" = "${current_branch}" ] || [ "${pick_commit}" = true ]; then
        echo_info "git --no-pager log ${object} --no-decorate --oneline -n 16\n"
    ...
    echo_info "$(echo "git checkout ${object} ${file_path}" | tr -s ' ')\n"
...
function clone() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'clone (repository) [-u username] [-s server] -b (bitbucket) -g (github) -l (gitlab) -H (https) -S (ssh) -h (help)\n'
    }
        ...
        while getopts :s:u:bglHSh option; do
            case "${option}" in
                ...
                h) echo_warning 'clone\n';
                    echo_success 'description:' 2 14; echo_primary 'Clone remote git repository to local folder\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -z "${GIT_SERVER}" ]; then
        echo_danger 'error: missing git server\n'
        _usage 2 8
        return 1
    fi
    ...
    if [ "${GIT_SERVER}" != 'gist.github.com' ]; then
        if [ -z "${GIT_USERNAME}" ]; then
            echo_danger 'error: missing git username\n'
            _usage
            return 1
        fi
    fi
    ...
    if [ "${GIT_SSH}" = true ] && [ "${GIT_SERVER}" = 'gist.github.com' ]; then
        echo_info "git clone \"git@${GIT_SERVER}:${git_repository}.git\"\n"
...
function conventional-branch() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'conventional-branch (branch name) -i (interactive) -r (rename) -t [type] -T [ticket] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :irt:T:h option; do
            case "${option}" in
                ...
                h) echo_warning 'conventional-branch\n';
                    echo_success 'description:' 2 14; echo_primary 'Create conventional branch name\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ "${interactive}" = true ]; then
        PS3=$(echo_success 'Please select type : ')
        ...
        if [ "${type}" = 'other' ]; then
            echo_success "Please enter type : [${default_type}] "
            read -r type
        fi

        echo_success "Please enter subject: [${default_subject}] "
        read -r subject

        echo_success "Please enter ticket number (optional): [${default_ticket}] "
        read -r ticket
    fi
    ...
    if [ "${rename}" = true ]; then
        ...
        echo_info "git branch -m ${type}${ticket}${subject}\n"
        eval "git branch -m \"${type}${ticket}${subject}\""

        return 0
    fi

    echo_info "git checkout -b ${type}${ticket}${subject}\n"
...
function conventional-commit() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'conventional-commit (subject) -r (reuse-message) -i (interactive) -t [type] -s [scope] -b [body] -T [ticket] -f [footer] -B (breaking change) -a [author] -A (default author) -d [date] -D (current date) -x (add all) -X (amend) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :Aa:Bb:Dd:f:ip:rs:T:t:Xxh option; do
            case "${option}" in
                ...
                x) add=true;;
                X) amend='--amend ';;
                h) echo_warning 'conventional-commit\n';
                    echo_success 'description:' 2 14; echo_primary 'Create conventional commit message\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -z "${default_subject}" ]; then
        EMOJIS=(☕ ⚡ ✨ ⭐ 🌟 🌶 🍩 🍬 🍭 🍰 🎉 🎊 🎯 🎷 🎸 🎺 🏁 🏅 🏆 🐀 🐁 🐄 🐅 🐇 🐈 🐕 🐝 🐠 🐧 🐰 🐱 🐼 👋 👍 👑 💙 💡 🔥 🚀 🛸 🤖 🤟 🥇 🦄 🦖)
        default_subject="${EMOJIS[ $((RANDOM % ${#EMOJIS[@]})) ]} $(date '+%Y-%m-%d %H:%M:%S')"
    fi
    ...
    if [ "${interactive}" = true ] && [ "${reuse_message}" = false ]; then
        PS3=$(echo_success 'Please select commit type : ')
        select type in "${valid_types[@]}"; do
            if [[ "${REPLY}" =~ ^[0-9]+$ ]] && [ "${REPLY}" -gt 0 ] && [ "${REPLY}" -le "${#valid_types[@]}" ]; then
                break 2;
            fi
        ...
        if [ "${type}" = 'other' ]; then
            echo_success "Please enter commit type : [${default_type}] "
            read -r type
        fi

        echo_success "Please enter scope (optional): [${default_scope}] "
        read -r scope
        ...
        echo_success "Please enter PR number (optional): [${default_pull_request}] "
        read -r pull_request

        echo_success "Please enter body (optional): [${default_body}] "
        read -r body

        echo_success 'Is breaking change: (yes/no) [no]: '
        read -r REPLY
        ...
        if [ "${breaking_change}" = true ]; then
            echo_success 'Please explain how change breaks current version (optional): '
            read -r footer
        fi
    fi
    ...
    echo_info "$(echo "git commit ${amend} ${message} ${author} ${date}" | tr -s ' ')\n"
...
function delete-github-workflows() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'delete-github-workflows -h (help)\n'
    }
    ...
    while getopts :h option; do
        case "${option}" in
            h) echo_warning 'delete-github-workflows\n';
                echo_success 'description:' 2 14; echo_primary 'Delete old Github workflows\n'
                _usage 2 14
                return 0;;
            \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
    ...
    if [ ! -x "$(command -v gh)" ]; then
        echo_danger 'error: gihub-cli required, visit: "https://github.com/cli/cli"\n'
        return 1
    fi
    ...
    if [ ! -x "$(command -v jq)" ]; then
        echo_danger 'error: jq required, enter: "sudo apt-get install -y jq" to install\n'
        return 1
    fi
...
function fetch() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'fetch -p (prune) -t (tags) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :pth option; do
            case "${option}" in
                p) prune=true;;
                t) tags=true;;
                h) echo_warning 'fetch\n';
                    echo_success 'description:' 2 14; echo_primary 'Fetch remote branches\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function gcontinue() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'gcontinue -h (help)\n'
    }
...
function get_url() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'get_url -b (branch) -n (new pull request) -p (pull request) -r (repository) -s (server) -u (username) -h (help)\n'
    }
    ...
    while getopts :bnprsuh option; do
        case "${option}" in
            ...
            h) echo_warning 'get_url\n';
                echo_success 'description:' 2 14; echo_primary 'Get repository urls from local config\n'
                _usage 2 14
                return 0;;
            \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
    ...
    if [ -z "${repository_url}" ]; then
        echo_danger "error: remote get-url is not available in local git repository\n"
        return 1
    fi
    ...
    if echo "${repository_url}" | grep -q -E '^(http://|https://|git@)'; then
        ...
        echo_danger "error: unknown url format in local git repository\n"
        return 1
    fi
    ...
    if [ "${get_new_pr}" = true ]; then
        if [ "${git_server}" = 'gist.github.com' ]; then
            echo_warning 'cannot get pull request url for gist\n'
            return 0
        fi
...
function greset() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'greset (commit_hash) -n [number] -o (from remote origin) -H (hard) -h (help)\n'
    }
        ...
        while getopts :123456789Hon:h option; do
            case "${option}" in
                ...
                h) echo_warning 'greset\n';
                    echo_success 'description:' 2 14; echo_primary 'Reset current branch to a previous commit\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ -n "${commit_hash}" ]; then
        if [ "$(_commit_exists "${arguments[${LBOUND}]}")" = false ]; then
            echo_danger "error: Invalid commit hash : \"${commit_hash}\"\n"
            _usage
            return 1
        fi
    fi
    ...
    # select commit hash
    if [ -z "${commit_hash}" ]; then
        echo_info 'git --no-pager log --no-decorate --oneline -n 16\n'
    ...
    if [ -n "${commit_hash}" ]; then
        if [ "$(_commit_exists "${commit_hash}")" = false ]; then
            echo_danger "error: Invalid commit : \"${commit_hash}\"\n"
            _usage
            return 1
        fi
    fi
...
function gstatus() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'gstatus -f (fetch) -vvv (verbose) -h (help)\n'
    }
...
function list-bitbucket() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'list-bitbucket (owner) -p [pages] -h (help)\n'
    }
        ...
        while getopts :p:h option; do
            case "${option}" in
                ...
                h) echo_warning 'list-bibucket\n';
                    echo_success 'description:' 2 14; echo_primary 'Lists bitbucket user public repositories, try browsing "https://bitbucket.org/repo/all"\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function list-github() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'list-github [owner] -v (verbose) -h (help)\n'
    }
        ...
        while getopts :vh option; do
            case "${option}" in
                ...
                h) echo_warning 'list-github\n';
                    echo_success 'description:' 2 14; echo_primary 'Lists GitHub user repositories\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function log() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'log (branch_name/commit_hash) -a [author] -f [file/folder path] -n [number] -B (pick branch) -C (pick commit) -D (no decorate) -l (list files only) -p (show patch) -P (no pager) -g (graph) -G (no graph) -O (name only) -o (oneline) -s (show stat) -S (no stat) -v (show status) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :123456789a:BCDf:gGln:oOpPhsSv option; do
            case "${option}" in
                ...
                v) stats='--name-status';;
                h) echo_warning 'log\n';
                    echo_success 'description:' 2 14; echo_primary 'Print git log\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ "${pick_commit}" = true ] && [ -z "${commit_hash}" ]; then
        echo_info "git --no-pager log ${branch} --no-decorate --oneline -n 16\n"
        commit_hash=$(_pick_commit "${branch}")
    fi
    # shellcheck disable=2181
    if [ $? != 0 ]; then
    ...
    echo_info "$(echo "git ${pager} log ${revision_range} ${author} ${stats} ${graph} ${no_decorate} ${no_summary} ${oneline} ${patch} ${file_path}" | tr -s ' ')\n"
...
function merge() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'merge (branch) [-m message] -a (abort) -c (continue) -h (help)\n'
    }
        ...
        while getopts :acm:h option; do
            case "${option}" in
                ...
                h) echo_warning 'merge\n';
                    echo_success 'description:' 2 14; echo_primary 'Merge git branch into current branch\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    echo_info "git merge ${branch} -m \"${message}\"\n"
...
function open-commit-files-in-ide() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'open-commit-files-in-ide (branch_name/commit_hash) -F [file type filter] -i [ide] -n [number] -B (pick branch) -C (pick commit)  -I (pick ide) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :123456789BCF:i:In:vh option; do
            case "${option}" in
                ...
                n) number="${OPTARG}";;
                v) verbose=true;;
                h) echo_warning 'open-commit-files-in-ide\n';
                    echo_success 'description:' 2 14; echo_primary 'Open modified/conflicting files in ide\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function pick() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'pick (commit_hash) -n (no-commit) -a (abort) -c (continue) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :acnh option; do
            case "${option}" in
                ...
                n) no_commit=true;;
                h) echo_warning 'pick\n';
                    echo_success 'description:' 2 14; echo_primary 'Apply a commit from another branch\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${no_commit}" = true ]; then
        echo_info "git cherry-pick --no-commit ${commit_hash}\n"
        git cherry-pick --no-commit "${commit_hash}"
    else
        echo_info "git cherry-pick ${commit_hash}\n"
...
function rebase() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'rebase (branch_name/commit_hash) -n [number] -a (abort) -c (continue) -s (self) -r (from root) -h (help)\n'
    }
        ...
        while getopts :123456789acn:rsh option; do
            case "${option}" in
                ...
                h) echo_warning 'rebase\n';
                    echo_success 'description:' 2 14; echo_primary 'Reorganize local commit history\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${root}" = true ]; then
        echo_info 'git rebase --interactive --root\n'
    ...
    if [ "${self}" = true ]; then
        echo_info "git rebase --interactive ${commit_hash}\n"
        git rebase --interactive "${commit_hash}"
        return 0
    fi

    echo_info "git rebase --interactive ${branch}\n"
...
function remote() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'remote (repository) [-s server] [-u username] -b (bitbucket) -g (github) -l (gitlab) -H (https) -S (ssh) -h (help)\n'
    }
        ...
        while getopts :s:u:bglHSh option; do
            case "${option}" in
                ...
                h) echo_warning 'remote\n';
                    echo_success 'description:' 2 14; echo_primary 'Set / print local git repository configuration\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ "${git_ssh}" = true ]; then
        ...
        echo_info "git remote \"${command}\" origin \"git@${git_server}:${git_username}/${git_repository}.git\"\n"
        git remote "${command}" origin "git@${git_server}:${git_username}/${git_repository}.git"
    else

        echo_info "git remote \"${command}\" origin \"https://${git_server}/${git_username}/${git_repository}\"\n"
...
function show() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'show (branch_name/commit_hash) -f [file path] -F [file type filter] -n [number] -B (pick branch) -C (pick commit) -l (list files only) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :123456789BCf:F:ln:vh option; do
            case "${option}" in
                ...
                v) verbose=true;;
                h) echo_warning 'show\n';
                    echo_success 'description:' 2 14; echo_primary 'Print changes from given commit\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function tag() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'tag [tag] [-m message] -c (commit) -f (fetch) -l (list) -p (push) -d (delete) -D (delete remote) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :c:m::Ddflph option; do
            case "${option}" in
                ...
                p) push=true;;
                h) echo_warning 'tag\n';
                    echo_success 'description:' 2 14; echo_primary 'Create, list or return latest tag\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -z "${tag}" ]; then
        if [ "${delete}" = true ] || [ "${delete_remote}" = true ]; then
            ...
            if [ "${list}" = true ]; then
                echo_info 'git --no-pager tag --list\n'
...
function convert-md() {
    ...
    if [ ! -f "${source}" ] && [ ! -d "${source}" ]; then
        echo_danger "error: source: \"${source}\" is invalid\n"
        _usage 2 8
    ...
    if [ ! -d "${source}" ] && [ "${recursive}" = true ]; then
        echo_danger "error: source: \"${source}\" is not a valid directory\n"
        _usage 2 8
...
function convert-to-mp3() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'convert-to-mp3 [file/folder] -r (recursive) -h (help)\n'
    }
    ...
    function _convert() {
        ...
        if [ -z "${CODEC}" ]; then
            echo_danger "error: cannot find valid codec for \"$1\"\n"
        ...
        # check file codec
        if [ "${CODEC}" = 'mp3' ]; then
            echo_warning "ignored : \"$1\" - already mp3 format\n"
        ...
        echo_info "ffmpeg -nostdin -hide_banner -i \"$1\" \"${NEW_PATH}\"\n"
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :rh option; do
            case "${option}" in
                r) recursive=true;;
                h) echo_warning 'convert-to-mp3\n';
                    echo_success 'description:' 2 14; echo_primary 'Convert audio or video to mp3\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -z "$(command -v 'ffmpeg')" ]; then
        echo_danger 'error: ffmpeg not installed, try "sudo apt-get install -y ffmpeg"\n'
        return 1
    fi
...
function mp3-rename-from-tag() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'mp3-rename-from-tag [folder] -a (%artist% - %title%) -d (%track%. %title%) -t (%track% %title%) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :adth option; do
            case "${option}" in
                ...
                h) echo_warning 'mp3-rename-from-tag\n';
                    echo_success 'description:' 2 14; echo_primary 'Rename mp3 from id3 tag (creates undo script)\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ -z "$(command -v 'id3tool')" ]; then
        echo_danger 'error: id3tool not installed, try "sudo apt-get install -y id3tool"\n'
        return 1
    fi
    ...
    echo_info "echo '#!/bin/bash' > \"${source}/mp3-rename-from-tag_undo.sh\"\n"
    ...
    find "${source}" -type f -name '*.mp3' | sort -t '\0' -n | while read -r file_path; do
        ...
        echo_warning "${artist} - ${year} ${album} - ${track}. ${title} - ${note}\n"
        ...
        if [ -z "${new_basename}" ]; then
            echo_info "could not retreive id3 tag from \"$(basename "${file_path}")\"\n"
            continue
        fi
...
function mp3-update-tag() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'mp3-update-tag [folder] -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'mp3-update-tag\n';
                    echo_success 'description:' 2 14; echo_primary 'Update mp3 tag from filename (creates undo script)\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    find "${source}" -type f -name '*.mp3' | while read -r file_path; do
        ...
        if [ -z "$(echo "${_basename}" | sed -E 's/^[0-9]+\. .+//')" ]; then
            ...
            # update file tag
            echo_info "id3tool --set-artist \"${artist}\" --set-album \"${album}\"  --set-year \"${year}\" --set-track \"${track}\" --set-title \"${title}\"  \"${file_path}\"\n"
...
function picture-get-names() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-get-names [file/folder] -v (verbose level) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :vh option; do
            case "${option}" in
                h) echo_warning 'picture-get-names\n';
                    echo_success 'description:' 2 14; echo_primary 'Print image subject names from file exif data\n'
                    _usage 2 14
                    return 0;;
                v) (( verbose++ ));;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
    ...
    if [ -z "$(command -v 'exiftool')" ]; then
        echo_danger 'error: exiftool not installed, try "sudo apt-get install -y exiftool"\n'
        return 1
    fi
    ...
    function _get_region_name() {
        if [ "${verbose}" -gt 1 ]; then
            echo_info "exiftool -RegionName \"$1\" | cut -d: -f2\n"
...
function picture-jpeg-optimize() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-jpeg-optimize [file/folder] -r (recursive) -h (help)\n'
    }
    ...
    function _jpegoptim() {
        echo_info "jpegoptim --strip-exif --all-progressive \"$1\"\n"
...
function picture-list-exif() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-list-exif [folder] -p (print to csv) -h (help)\n'
    }
        ...
        while getopts :hp option; do
            case "${option}" in
                ...
                h) echo_warning 'picture-list-exif\n';
                    echo_success 'description:' 2 14; echo_primary 'Lists picture exif modified at\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ "${print}" = true ]; then
        ...
        if [ -x "$(command -v subl)" ]; then
            ...
            echo_info "${VISUAL} \"${source}/picture-list-exif.csv\"\n"
...
function picture-organize() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-organize [folder] -e (use exif) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :eh option; do
            case "${option}" in
                e) exif=true;;
                h) echo_warning 'picture-organize\n';
                    echo_success 'description:' 2 14; echo_primary 'Organise pictures and videos by last modified date into folders (creates undo script)\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function picture-png2jpg() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-png2jpg [file/folder] -r (recursive) -h (help)\n'
    }
...
function picture-rename-to-datetime() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-rename-to-datetime [folder] -e (use exif) -a (append filename) -h (help)\n'
    }
        ...
        while getopts :aeh option; do
            case "${option}" in
                ...
                h) echo_warning 'picture-rename-to-datetime\n';
                    echo_success 'description:' 2 14; echo_primary 'Rename pictures to date last modified (creates undo script)\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
...
function picture-resize() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-resize [file/folder] -r (recursive) -s (size default=50%) -h (help)\n'
    }
...
function picture-update-datetime() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'picture-update-datetime [folder] -e (update exif) -h (help)\n'
    }
        ...
        while getopts :eh option; do
            case "${option}" in
                ...
                h) echo_warning 'picture-update-datetime\n';
                    echo_success 'description:' 2 14; echo_primary 'Update picture datetime from filename (creates undo script)\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    find "${source}" -type f -iregex '.+\.\(3g2\|3gp\|aax\|ai\|arw\|asf\|avi\|cr2\|cr3\|crm\|crw\|cs1\|dcp\|dng\|dr4\|dvb\|eps\|erf\|exif\|exv\|f4a\|f4v\|fff\|flif\|gif\|gpr\|hdp\|icc\|iiq\|ind\|jng\|jp2\|jpeg\|jpg\|m4a\|m4v\|mef\|mie\|mng\|mod\|mos\|mov\|mp4\|mpeg\|mpg\|mpo\|mqv\|mrw\|nef\|nrw\|orf\|pbm\|pdf\|pef\|pgm\|png\|png\|ppm\|ps\|psb\|psd\|qtif\|raf\|raw\|rw2\|rwl\|sr2\|srw\|thm\|tiff\|vrd\|wdp\|wmv\|x3f\|xmp\)$' | while read -r file_path; do
        ...
        if [ -z "${datetime}" ]; then
            echo_danger "error: invalid datetime: ${date} ${time}\n"
            continue
        fi
...
function rename-extension-to-codec() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'rename-extension-to-codec [folder] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'rename-extension-to-codec\n';
                    echo_success 'description:' 2 14; echo_primary 'Change multimedia file to correct extension\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    find "${source}" -type f -iregex ".+\.\(aac\|aiff\|flac\|m4a\|mp3\|ogg\|wav\|wma\|avi\|flv\|m2ts\|mkv\|mov\|mp4\|mts\|webm\|wmv\)" | while read -r file_path; do
        ...
        # check codec is valid
        if [ -z "${codec}" ]; then
            echo_danger "error: cannot find valid codec for \"${file_path}\"\n"
        ...
        if [ ".${codec}" = "${extension}" ]; then
            echo_warning "ignored : \"${file_path}\" - already correct extension\n"
            continue
        fi
        ...
        # -nostdin : avoid ffmpeg to swallow stdin
        echo_info "mv \"${file_path}\" \"${new_path}\"\n"
...
alias local-ip='echo_info "hostname -I\n"; hostname -I' ## Print local ip
alias open-ports='echo_info "lsof -i -Pn | grep LISTEN\n"; lsof -i -Pn | grep LISTEN' ## List open ports
...
alias resolve='echo_info "host\n" &&; host'             ## Resolve reverse hostname
alias iptables-list-rules='echo_info "sudo iptables -S"; sudo iptables -S; echo_info "sudo iptables -L\n"; sudo iptables -L' ## list iptables rules
alias start-vnc='echo_info "x11vnc -usepw -bg -forever\n"; x11vnc -usepw -bg -forever' ## Start VNC server in the background
alias stop-vnc='echo_info "x11vnc -remote stop\n"; x11vnc -remote stop' ## Start VNC server in the background
...
function external-ip() {
    ...
    if [ -z "$(command -v 'wget')" ] && [ -z "$(command -v 'curl')" ]; then
        echo_danger 'error: curl not installed, try "sudo apt-get install -y curl"\n'
        return 1
    fi
...
function list-adapters() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'list-adapters -c (filter connected devices) -w (filter wifi devices) -v (verbose) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :cwvh option; do
            case "${option}" in
                ...
                w) filter_wifi=true;;
                v) verbose=true;;
                h) echo_warning 'list-adapters\n';
                    echo_success 'description:' 2 14; echo_primary 'List network adapters\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
    ...
    if [ "${filter_connected}" = true ]; then
        ...
        if [ -z "$(command -v 'arp')" ]; then
            echo_danger 'error: arp not installed, try "sudo apt-get install -y net-tools"\n'
            return 1
        fi
        ...
        if [ -z "$(command -v 'iwconfig')" ]; then
            echo_danger 'error: iwconfig not installed, try "sudo apt-get install -y wireless-tools"\n'
            return 1
        fi
...
function mount-nfs() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'mount-nfs [server]:[resource] -m [mount_point] -h (help)\n'
    }
        ...
        while getopts :m:h option; do
            case "${option}" in
                ...
                h) echo_warning 'mount-nfs\n';
                    echo_success 'description:' 2 14; echo_primary 'Mount nfs shared ressource into local mount point\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if ! dpkg -la 2>/dev/null | grep -q nfs-common; then
        echo_danger 'error: nsf-common required: enter "sudo apt-get install -y nfs-common" to install\n'
        return 1
    fi
...
function port-redirect() {
    ...
    if [ ! -x "$(command -v iptables)" ]; then
        echo_danger 'error: iptables required, enter: "sudo apt-get install -y iptables" to install\n'
        return 1
    fi
...
function scan() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'scan [range/ip] -q (quick) -p (ping) -P (port) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :hpPq option; do
            case "${option}" in
                ...
                h) echo_warning 'scan\n';
                    echo_success 'description:' 2 14; echo_primary 'Scan with nmap\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ ! -x "$(command -v nmap)" ]; then
        echo_danger 'error: nmap required, enter: "sudo apt-get install -y nmap" to install\n'
        return 1
    fi
...
function wifi-get-bssids() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-get-bssids -h (help)\n'
    }
        ...
        while getopts :a:h option; do
            case "${option}" in
                ...
                h) echo_warning 'wifi-get-bssids\n';
                    echo_success 'description:' 2 14; echo_primary 'Print available bssids, channels and ssids\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${wifi_adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
    ...
    if [ -z "$(command -v 'airodump-ng')" ]; then
        echo_danger 'error: airodump-ng not installed, try "sudo apt-get install -y aircrack-ng"\n'
        return 1
    fi
    ...
    echo_info "timeout 5s sudo airodump-ng --write \"${temp_dir}\"/temp --output-format csv --band abg \"${wifi_adapter}\" &>/dev/null\n"
    timeout 5s sudo airodump-ng --write "${temp_dir}"/temp --output-format csv --band abg "${wifi_adapter}" &>/dev/null

    echo_info "cat \"${temp_dir}\"/temp-*.csv | sed -E 's/, +/,/g' | sed -E '/^ $/d' | tail -n +2 | head -n -1 | cut -d, -f1,4,14\n"
...
function wifi-monitor-mode() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-monitor-mode (adapter) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'wifi-monitor-mode\n';
                    echo_success 'description:' 2 14; echo_primary 'Set wlan adapter to monitor mode\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_primary "available wifi adapters: ${wifi_adapters}\n"
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
...
function wifi-radar() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-radar (adapter) -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'wifi-radar\n';
                    echo_success 'description:' 2 14; echo_primary 'Discover available wireless networks\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_primary "available wifi adapters: ${wifi_adapters}\n"
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
...
function php-server() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'php-server (docroot) -i (ip) -p (port) -h (help)\n'
    }
        ...
        while getopts :i:p:h option; do
            case "${option}" in
                ...
                h) echo_warning 'php-server\n';
                    echo_success 'description:' 2 14; echo_primary 'Start php builtin server\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ ! -x "$(command -v php)" ]; then
        echo_danger 'error: php required, enter: "sudo apt-get install -y php" to install\n'
        return 1
    fi
...
function set-php-version() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'set-php [version] -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'set-php-version\n';
                    echo_success 'description:' 2 14; echo_primary 'Set default php version\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ "${is_valid}" = false ]; then
        echo_danger 'error: Invalid PHP version\n'
        _usage 2 8
        return 1
    fi
...
alias pytests='echo_info "python3 -m unittest -v\n" && python3 -m unittest -v' ## Perform python unittest
...
function change-mac-address() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'change-mac-address (adapter) -s [spoof_mac] -h (help)\n'
    }
        ...
        while getopts :s:h option; do
            case "${option}" in
                ...
                h) echo_warning 'change-mac-address\n';
                    echo_success 'description:' 2 14; echo_primary 'Spoof network adapter mac address (generates random mac if not provided)\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
    ...
    if [[ ! "${spoof}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        echo_danger "error: invalid mac address \"${spoof}\"\n"
        _usage 2 8
        echo_success 2 8 'note:'; echo_warning "available adapters: ${adapters}\n"
        return 1
    fi
    ...
    if [ -z "${adapter}" ]; then
        PS3=$(echo_success 'Please select adapter : ')
...
function netcat-listen()  {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'netcat-listen (port) -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'netcat-listen\n';
                    echo_success 'description:' 2 14; echo_primary 'Listen to reverse shell connection with netcat\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function reverse-shell() {
    ...
    if [[ ! "${rhost}" =~ ^([0-9]{0,3}\.){3}[0-9]{0,3}$ ]]; then
        echo_danger 'error: host should be an ipv4 address\n'
        _usage 2 8
        return 1
    fi
...
function wifi-deauth() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-deauth (adapter) -r [router_mac] -c [client_mac] -p [packet_count] -h (help)\n'
    }
        ...
        while getopts :c:p:r:h option; do
            case "${option}" in
                ...
                h) echo_warning 'wifi-deauth\n';
                    echo_success 'description:' 2 14; echo_primary 'Disconnect client from wifi network\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${wifi_adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
    ...
    if [ -z "$(command -v 'aireplay-ng')" ]; then
        echo_danger 'error: aireplay-ng not installed, try "sudo apt-get install -y aircrack-ng"\n'
        return 1
    fi
    ...
    if [[ ! "${router}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        echo_danger "error: wrong mac address format ${router}\n"
        _usage 2 8
        return 1
    fi
    ...
    if [[ ! "${client}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        echo_danger "error: wrong mac address format ${client}\n"
        _usage 2 8
        return 1
    fi
...
function wifi-fakeauth() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-fakeauth (adapter) -r [router_mac] (-c client_mac) (-d delay) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :c:d:r:h option; do
            case "${option}" in
                ...
                d) delay="${OPTARG}";;
                r) router="${OPTARG}";;
                h) echo_warning 'wifi-fakeauth\n';
                    echo_success 'description:' 2 14; echo_primary 'Associates with target wifi network\n'
                    _status 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${wifi_adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
...
function wifi-sniff() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'wifi-sniff [target] [-c channel] (-a adapter) -n (no_write) -h (help)\n'
    }
        ...
        while getopts :c:nh option; do
            case "${option}" in
                ...
                h) echo_warning 'wifi-sniff\n';
                    echo_success 'description:' 2 14; echo_primary 'Dump captured packets from target adapter\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning "available adapters: ${wifi_adapters}\n"
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
    ...
    if [[ ! "${target}" =~ ^([a-zA-Z0-9]{2}:){5}[a-zA-Z0-9]{2}$ ]]; then
        echo_danger "error: wrong mac address format ${target}\n"
        _usage 2 8
        return 1
    fi
...
function decode() {
    ...
    if [ ! -x "$(command -v base64)" ]; then
        echo_danger 'error: base64 required, enter: "sudo apt-get install -y base64" to install\n'
        return 1
    fi
    ...
    if [ -z "${*}" ]; then
        echo_danger 'error: some mandatory argument missing\n'
...
function random-string() {
    ...
    if [[ ! "${length}" =~ [0-9] ]]; then
        echo_danger "error: invalid argument ($1)\n"
        _usage 2 8
        return 1
    fi
...
function urldecode() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'urldecode -h (help)\n'
    }
    ...
    while getopts :h option; do
        case "${option}" in
            h) echo_warning 'urldecode\n';
                echo_success 'description:' 2 14; echo_primary 'Decode string froml URL format\n'
                _usage 2 14
                return 0;;
            \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
    ...
    if [ "${SHELL}" != '/bin/bash' ]; then
        echo_danger 'error: your shell is not compatible with this function\n'
        return 1
    fi
...
function sf() {
    ...
    if [ -x ./app/console ]; then
        ...
        echo_danger 'error: no symfony console executable found\n'
        return 1
    fi
}
...
function sf-dump-server() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'sf-server-dump -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'sf-dump-server\n';
                    echo_success 'description:' 2 14; echo_primary 'Start project debug server\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function backup() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'backup [source] [destination] -h (help)\n'
    }
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'backup\n';
                    echo_success 'description:' 2 14; echo_primary 'Synchronize destination folder with source with rsync\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ ! -d "${source}" ]; then
        echo_danger 'error: source folder not found\n'
        _usage
    fi

    if [ ! -d "${destination}" ]; then
        echo_danger 'error: destination folder not found\n'
        _usage
    fi
    ...
    echo_warning "Copying \"${source}\" to \"${destination}\"\n"
    echo_warning "Some data in \"${destination}\" may be lost\n"
    echo_success 'Are you sure you want to continue ? (yes/no) [no]: '
    read -r USER_PROMPT
    ...
    echo_info "rsync -avuz \"${source}\" \"${destination}\" --delete --exclude='/.*' --exclude='/snap' --exclude='/VirtualBox VMs'\n"
...
function flash-bootable-iso() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'flash-bootable-iso [source] [device (eg: /dev/sdb)] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'flash-bootable-iso\n';
                    echo_success 'description:' 2 14; echo_primary 'Create bootable usb drive from iso file or generate iso file from source\n'
                    _usage 2 14
                    echo_success 'note:' 2 14; echo_warning 'List available drives with "drives" alias\n'
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
            esac
    ...
    if [ -z "${source}" ]; then
        echo_danger 'error: source parameter is missing\n'
        _usage
    fi

    if [ -z "${destination}" ]; then
        echo_danger 'error: destination parameter is missing\n'
        _usage
    fi

    echo_warning "Writing \"${source}\" to \"${destination}\"\n"
    ...
    echo_info "sudo dd if=\"${source}\" of=\"${destination}\" bs=16M oflag=direct status=progress\n"
    sudo dd if="${source}" of="${destination}" bs=16M oflag=direct status=progress

    echo_warning "bs=16M: This sets the block size (bs) for the dd command. The dd command will read and write up to 16M bytes at a time.\n"
    echo_warning "oflag=direct: This option tells dd to try and use the most efficient method to write data to the destination. The direct option attempts to bypass the filesystem buffer and write directly to the storage device, which can improve performance for raw data copying.\n"
...
function mod() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'mod (source) -m [mode] -r (recursive) -d (default=664) -x (executable=775) -X (executable=777) -s (sudo) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :drsxXm:h option; do
            case "${option}" in
                ...
                m) mode="${OPTARG}";;
                h) echo_warning 'mod\n';
                    echo_success 'description:' 2 14; echo_primary 'Change files mode recursively\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [[ ! "${mode}" =~ ^[0-7]+$ ]]; then
        echo_danger 'error: some mandatory parameter is invalid\n'
        _usage 2 8
        return 1
    fi
...
function own() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'own (source) -u [user] -g [group] -n (nobody:nogroup) -R (root) -r (recursive) -s (sudo) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :nrRsu:g:h option; do
            case "${option}" in
                ...
                g) group="${OPTARG}";;
                h) echo_warning 'own\n';
                    echo_success 'description:' 2 14; echo_primary 'Own files and folders\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    if [ -d "${source}" ] && [ "${recursive}" = true ]; then
        echo_info "${command} \"${user}:${group}\" -R \"${source}\"\n"
    ...
    if [ -d "${source}" ] && [ "${recursive}" = false ]; then
        echo_info "${command} \"${user}:${group}\" \"${source}\"\n"
...
function open() {
    ...
    for ARGUMENTS in "${@}"; do
        case "${OSTYPE}" in
            ...
            *) echo_danger "error: ostype: \"${OSTYPE}\" is not handled\n"; return 1;;
        esac
    done
}
...
function change-extensions() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'change-extensions [folder] -e [old extension] -n [new extension] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :e:n:h option; do
            case "${option}" in
                ...
                h) echo_warning 'change-extensions\n';
                    echo_success 'description:' 2 14; echo_primary 'change files extensions from given folder\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
    ...
    find "${source}" -maxdepth 1 -type f -name "*.${old_extension}" | while read -r file_path; do
        ...
        if [ -f "${new_path}" ]; then
            echo_danger 'error: file already exists.\n'
        else
            # rename file
            echo_info "mv -nv \"${file_path}\" \"${new_path}\"\n"
            mv -nv "${file_path}" "${new_path}"
...
function compress() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'compress [source] [destination] -r (recursive) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :hr option; do
            case "${option}" in
                r) recursive=true;;
                h) echo_warning 'compress\n';
                    echo_success 'description:' 2 14; echo_primary 'Compress a file, a folder or each subfolders into separate archives recursively \n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ ! -f "${source}" ] && [ ! -d "${source}" ]; then
        echo_danger "error: \"${source}\" is invalid\n"
        _usage 2 8
        return 1
    fi
    ...
    if [ ! -d "${destination}" ]; then
        echo_danger "error: \"${destination}\" is not a valid directory\n"
        _usage 2 8
        return 1
    fi
...
function create-desktop-shortcut() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'create-desktop-shortcut [executable] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'clean-folder\n';
                    echo_success 'description:' 2 14; echo_primary 'Create a shortcut on user destop\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
    ...
    if [ ! -x "${source}" ]; then
        echo_danger "error: \"${source}\" is not executable\n"
        _usage 2 8
        return 1
    fi
...
function help() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'help [command] (flag)\n'
    }
...
function move-all-files-here() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'move-all-files-here [source] -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) echo_warning 'compress\n';
                    echo_success 'description:' 2 14; echo_primary 'Move all files from subfolders into current folder (no overwrite)\n'
                    _usage 2 14
                    return 0;;
                \?) echo_danger "error: invalid option \"${OPTARG}\"\n"
                    return 1;;
...
function rename() {
    function _usage() {
        echo_success 'usage:' "$1" "$2"; echo_primary 'rename [source] -r (recursive) -a (preserve accents) -e (remove extra char) -s (snake_case) -t (train-case) -l (lower case) -u (upper CASE) -h (help)\n'
    }
    ...
    function _rename() {
        ...
        # mv -n = do not overwrite
        echo_info "mv -n \"$1\" \"${PARENT}/${NEW_NAME}\"\n"
        ...
        while getopts :eluprsth option; do
            case "${option}" in
                ...
                h) echo_warning 'rename\n';
                    echo_success 'description:' 2 14; echo_primary 'Rename files recursively\n'
                    _usage 2 14
                    return 0;;
                :) echo_danger "error: \"${OPTARG}\" requires value\n"
                    return 1;;
